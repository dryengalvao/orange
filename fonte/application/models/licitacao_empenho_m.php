...
class licitacao_empenho_m extends CI_Model{
    ...
    public function lista_licitacao_empenho(){
        ...
        $this->db->from('licitacao_empenho');
		$this->db->join('licitacao','licitacao.id = licitacao_empenho.id_licitacao');
		$this->db->join('meses', 'meses.id = licitacao_empenho.mes');
    ...
    public function exportar($dados){
        $consulta = 'SELECT ce.*, c."nu_licitacao" FROM licitacao_empenho ce JOIN licitacao c on c.id = ce.id_licitacao WHERE';
